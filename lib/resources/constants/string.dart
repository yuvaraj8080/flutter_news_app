class AppStrings {
  static const appname = "News";
  static const poweredBy = "yuvaraj Dekhane";

  //onboarding
   static const onboardingTitle0 = "Explore Latest News";
   static const skip = "Skip";
   static const onboardingTitle1 = "Get Notified First";
   static const onboardingTitle2 = "Save Articles";
   static const onboardingDesc0 = "Explore latest news and events happenig in the word in a minute...";
   static const onboardingDesc1 = "Subscribe and get notified to all of our article & share with your friends...";
   static const onboardingDesc2 = "Booking, like & comment on your favourite article and share on social media...";

   ///AUTHENTICATION
   static const  welcome = "Welcome to News";
   static const  signInToContinue = "Sign in to Continue";
   static const  signInGoogle = "Sign in with Google";
   static const  signInFB = "Sign in with Facebook";
   static const  donHaveAccount = "Don't have an Account";
   static const  continueWithEmail = "Continue with Email";
   static const  privacyPolicyTermCondition = "By signing in, you agree to our";
   static const  privacyPolicy = "Privacy Policy";
   static const  termCondition = "Term of Services";

   /// LOGIN
    static const login = "Login";
    static const follSimple = "Follow the simple steps";
    static const email = "Email Address";
    static const emailHint = "username@gmail.com";
    static const passwordHint = "********";
    static const password = "password";
    static const signup = "Signup";


}